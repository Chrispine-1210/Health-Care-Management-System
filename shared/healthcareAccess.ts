...
export const ROLE_PERMISSIONS: Record<HealthcareRole, ReadonlySet<Permission>> = {
  patient: new Set([p.PATIENT_PROFILE_READ, p.PATIENT_PROFILE_UPDATE, p.APPOINTMENT_READ, p.APPOINTMENT_MANAGE, p.PRESCRIPTION_READ, p.ORDER_READ, p.PAYMENT_READ, p.PAYMENT_INITIATE]),
  ...
  pharmacist: new Set([p.PATIENT_PROFILE_READ, p.PRESCRIPTION_READ, p.PRESCRIPTION_DISPENSE, p.ORDER_READ, p.ORDER_MANAGE, p.EMERGENCY_ACCESS_REQUEST]),
  receptionist: new Set([p.PATIENT_PROFILE_READ, p.APPOINTMENT_READ, p.APPOINTMENT_MANAGE, p.ORDER_READ, p.NOTIFICATION_SEND]),
  ...
  branch_administrator: new Set([p.ORDER_READ, p.ORDER_MANAGE, p.DELIVERY_READ, p.STAFF_MANAGE_BRANCH, p.BRANCH_MANAGE, p.NOTIFICATION_SEND, p.REPORT_VIEW, p.AUDIT_LOG_VIEW]),
  system_administrator: new Set([p.STAFF_MANAGE_SYSTEM, p.BRANCH_MANAGE, p.NOTIFICATION_SEND, p.REPORT_VIEW, p.AUDIT_LOG_VIEW, p.SYSTEM_CONFIGURE]),
  super_administrator: new Set([p.STAFF_MANAGE_SYSTEM, p.BRANCH_MANAGE, p.NOTIFICATION_SEND, p.REPORT_VIEW, p.DATA_EXPORT, p.AUDIT_LOG_VIEW, p.SYSTEM_CONFIGURE]),
};
