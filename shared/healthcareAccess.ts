...
export const ROLE_PERMISSIONS: Record<HealthcareRole, ReadonlySet<Permission>> = {
  ...
  pharmacist: new Set([p.PATIENT_PROFILE_READ, p.PRESCRIPTION_READ, p.PRESCRIPTION_DISPENSE, p.ORDER_READ, p.ORDER_MANAGE, p.INVENTORY_READ, p.INVENTORY_MANAGE, p.EMERGENCY_ACCESS_REQUEST]),
  receptionist: new Set([p.PATIENT_PROFILE_READ, p.APPOINTMENT_READ, p.APPOINTMENT_MANAGE, p.ORDER_READ, p.ORDER_MANAGE, p.INVENTORY_READ, p.NOTIFICATION_SEND]),
  ...
  branch_administrator: new Set([p.ORDER_READ, p.ORDER_MANAGE, p.INVENTORY_READ, p.INVENTORY_MANAGE, p.PRODUCT_MANAGE, p.DELIVERY_READ, p.STAFF_MANAGE_BRANCH, p.BRANCH_MANAGE, p.CONTENT_MANAGE, p.NOTIFICATION_SEND, p.REPORT_VIEW, p.AUDIT_LOG_VIEW]),
