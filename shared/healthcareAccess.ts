...
export const ROLE_PERMISSIONS: Record<HealthcareRole, ReadonlySet<Permission>> = {
  ...
  pharmacist: new Set([p.PATIENT_PROFILE_READ, p.PRESCRIPTION_READ, p.PRESCRIPTION_DISPENSE, p.PRESCRIPTION_REVIEW, p.PRESCRIPTION_APPROVE, p.PRESCRIPTION_REJECT, p.DISPENSING_START, p.DISPENSING_COMPLETE, p.ORDER_READ, p.ORDER_MANAGE, p.ORDER_CANCEL, p.ORDER_CANCEL_AFTER_PAYMENT, p.RESERVATION_RELEASE, p.INVENTORY_READ, p.INVENTORY_MANAGE, p.EMERGENCY_ACCESS_REQUEST]),
